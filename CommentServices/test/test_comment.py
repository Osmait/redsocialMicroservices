...
def test_create(comment_service):
    ...
    # Utiliza patch para reemplazar el comportamiento real de
    # CommentDto.model_dump
    with patch.object(mock_comment, "model_dump", return_value={}):
        # Configura el método add de la base de datos para retornar el Mock de
        # comentario de base de datos
        comment_service.db.add.return_value = mock_db_comment
        # Ejecuta la función create
        result = comment_service.create(mock_comment)
    # # Verifica que el método commit de la base de datos se haya
    # llamado una vez
    comment_service.db.commit.assert_called_once()

    # Verifica que la función create retorne None (o el valor que decidas
    # retornar en CommentService.create)
...
def test_find_all_by_id(comment_service):
    ...
    # Configura la función filter para devolver los comentarios simulados
    # según el post_id
    comment_service.db.query(Comment).filter(
        Comment.post_id == "post456"
    ).all.return_value = [mock_comment]

    # Llama a la función find_all_by_id con un post_id específico
    comments = comment_service.find_all_by_id("post456")

    # Verifica que la función devuelva los comentarios correctos
    # según el post_id
